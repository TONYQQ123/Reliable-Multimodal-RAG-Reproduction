...
def preprocess_v1(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, chosen: int = 1) -> Dict:
    ...
    if has_image:
        ...
        prompt_input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in prompts], dim=0)
    else:
        ...
        prompt_input_ids = tokenizer(prompts, return_tensors="pt", padding="longest", max_length=tokenizer.model_max_length, truncation=True).input_ids
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, chosen: int = 1) -> Dict:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=('image' in self.list_data_dict[i]), chosen=1)
        if isinstance(i, int):
        ...
        if isinstance(i, int):
            sources = [sources]
        if 'image' in sources[0]:
            rejected_sources = preprocess_multimodal(copy.deepcopy([e["rejected_conversations"] for e in sources]), self.data_args)
        else:
            rejected_sources = copy.deepcopy([e["rejected_conversations"] for e in sources])
            
        rejected_data_dict = preprocess(rejected_sources, self.tokenizer, has_image=('image' in self.list_data_dict[i]), chosen=0)
        if isinstance(i, int):
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        prompt_input_ids = torch.nn.utils.rnn.pad_sequence(prompt_input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
...
def train():
    ...
    # Initialize Vision Modules for Policy Model
    if model_args.vision_tower is not None:
    ...
    # --- 修正：初始化 Reference Model 的 Vision Tower ---
    # 如果不加這段，ref_model 會沒有 vision_tower 屬性，導致 DPO forward 時報錯
    if model_args.vision_tower is not None:
        model_ref.get_model().initialize_vision_modules(model_args=model_args, fsdp=training_args.fsdp)
        vision_tower_ref = model_ref.get_vision_tower()
        vision_tower_ref.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        # 注意：我們不需要重新設定 data_args，因為 policy model 已經設過了
        # 但我們需要載入 weights
